...
def run():
    ...
    eval = model_orig.solve_ham(k_vec, return_eigvecs=False)
    n_bands = eval.shape[-1]

    wf = WFArray(model_orig, [numk])
    ...
    eval = model_perp.solve_ham(k_vec, return_eigvecs=False)
    n_bands = eval.shape[-1]

    wf = WFArray(model_perp, [numk])
    wf.solve_on_grid([0.0])
    n_occ = n_bands // 2
