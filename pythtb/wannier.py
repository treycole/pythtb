...
class Wannier():
    def __init__(
            self, model: "TBModel", energy_eigstates: Bloch, *nks
            ):
        ...
        assert hasattr(self.energy_eigstates, 'is_energy_eigstate'), "Energy eigenstates must be solved with 'solve_model' before Wannierization"
        ...
        self.supercell = (
            np.stack(mesh, axis=-1)      # (..., len(nks))
            .reshape(-1, len(nks))     # (product, dims)
        )
